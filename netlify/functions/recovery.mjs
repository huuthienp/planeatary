...
export async function handler(event, context) {
    console.warn('Please use `env.URL` for `strongBlobUrl` in production');
    const eventTime = Date.now();  // v2
    try {  // v2
        ...
        /* Pre-Processing */
        connectLambda(event);  // v2
        const eventBody = JSON.parse(event.body);
        const { identity } = context.clientContext; // v2
        const { env } = process;
